...
> Тип задаётся на приёме и определяет МЕХАНИКУ, а не только слова: какие правила прилетят
> контрактным агентам, какой уровень effort будет выбран, потребуются ли специальные проверки,
> и может ли кодер вообще запускаться. Он виден в статус-строке. Не угадывай - если сомневаешься
> между двумя, бери более тяжёлый.
